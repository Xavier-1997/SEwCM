...
parser.add_argument('--margin', default=0.5, type=float,
                    metavar='M', help='margin for softmax')
...
src_dir = 'E:/Unsupervised_Embedding_Learning/data/'
...
suffix = suffix + '_temp_{}_km_{}_alr'.format(args.batch_t, args.arch)
...
criterion = BatchCriterion(args.margin, args.batch_t, args.batch_size)
...
if args.test_only or len(args.resume)>0:
    ...
    checkpoint = torch.load(model_path + 'cub_batch_64_pret_temp_0.1_km_1_inception_v1_ml_alr_best_margin_0.5.t')
...
for epoch in range(start_epoch, start_epoch+50):
    ...
    if args.dataset =='cub200':
        ...
        temploader = torch.utils.data.DataLoader(trainset, batch_size=args.test_batch, shuffle=False, num_workers=0)
    ...
    #df = pd.DataFrame(testLabels)
    #df.to_csv('testLabels.csv', index=False)
