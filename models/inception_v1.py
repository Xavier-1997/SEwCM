...
def inception_v1_ml(pretrained=False, low_dim=128):
    base_model = inception_v1_googlenet()
    base_model_weights_path = 'E:/Unsupervised_Embedding_Learning/models/inception.pth'
    if os.path.exists(base_model_weights_path):
        base_model.load_state_dict(
            {k: torch.from_numpy(v).cuda() for k, v in torch.load(base_model_weights_path).items()})
